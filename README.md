...
6 つとも最初から**空**で用意されているので、消してから貼る必要はありません。
入力例が 2 つしかない問題では `input3.txt` / `expected3.txt` を空のままにしておけば、
採点時にその対はスキップされ、不正解にはなりません (入力例が 1 つの問題も同様)。
全部空のまま採点すると、入力なしで 1 回だけ実行して出力を見せます (判定なし)。
